...
> **Внимание:** реализована оркестрация цикла, но не предметная работа стадий. Прогон
> `make intent-run` проводит намерение через все шесть стадий, ведёт состояние, итерации
> и отчёт, однако сами стадии выполняются подставными шагами.

Что реализовано:

- **движок цикла** — порядок стадий, переходы состояний, оба обратных ребра, лимит итераций
  и прерывание по его исчерпании, отчёт прогона, коды возврата;
- **сценарий прогона** — файл, задающий исход каждой стадии по итерациям и вход цикла;
  им описывается любая ветвь схемы, включая отклонение, останов и исчерпание лимита.

Что стадии сегодня не делают:

- стадия 1 — INT как артефакт не читается: код намерения и его позиции берутся из сценария;
- стадия 2 — валидация INT выполняется человеком при чтении заявки;
- стадии 3–5 — правка модели, выпуск и накат выполняются действующими командами вручную;
- верхний слой стадии 6 — сверка «намерение ↔ факт» выполняется вручную; машинно работает
  только нижний слой, `make model-verify`.

Предметная реализация каждой стадии встаёт на место своего подставного шага отдельным
изменением: контракт шага и движок при этом не меняются.
...
Лимит итераций по умолчанию — **3**, переопределяется параметром запуска.
...
| 1.1.0 | 2026-09-20 | OpenSpec change `add-intent-pipeline` | Claude Code | Claude Opus 5 | Раздел ограничений приведён к реализованному состоянию: оркестрация цикла реализована — порядок стадий, переходы состояний, обратные рёбра, лимит итераций, отчёт и коды возврата, — а стадии выполняются подставными шагами, исход которых задаёт сценарий прогона. Перечень непокрытого пересобран: стадия 1 читает вход из сценария, стадии 3–5 выполняются вручную действующими командами. Зафиксировано значение лимита по умолчанию — 3, и порядок замены подставного шага предметным. Из открытых вопросов убрано значение лимита |
