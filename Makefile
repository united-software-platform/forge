...
# Прогон цикла намерения по сценарию. Шаги стадий подставные: предметной работы цикл
# не выполняет, хранилище ему не нужно. Код возврата 2 означает незамкнутый цикл —
# останов или прерывание по лимиту, — а не сломанное окружение.
intent-run: ## Прогнать цикл намерения по сценарию (SCENARIO=..., LIMIT=...)
	@$(model_run) intent run \
		--scenario "$${SCENARIO:-model/intents/scenarios/demo.yaml}" \
		$${LIMIT:+--max-iterations $$LIMIT}
